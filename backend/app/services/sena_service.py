...
class SenaService:
    ...
    def obtener_persona_local_o_sena(db: Session, persona_id: str):
        ...
        # 3. Fallback: buscar en la BD erp_sena (Postgres) directamente.
        #    Se usa cuando no hay API NestJS configurada (entorno local/demo).
        if persona_id:
            try:
                from app.database.sena_db import (
                    buscar_persona_por_uuid,
                    buscar_persona_por_cedula,
                )
                p = None
                if str(persona_id).isdigit():
                    p = buscar_persona_por_cedula(str(persona_id))
                else:
                    p = buscar_persona_por_uuid(str(persona_id))
                if p:
                    return {
                        "id": p["fk_persona"],
                        "nombre": p["nombre"],
                        "documento": p["documento"],
                        "rol": p.get("rol", "aprendiz"),
                        "programa": p.get("programa", "SENA"),
                        "tipo_propietario": p.get("rol", "aprendiz"),
                    }
            except Exception as e:
                print(f"Error fallback erp_sena para {persona_id}: {e}")
